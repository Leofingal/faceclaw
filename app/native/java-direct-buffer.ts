 ...
 *
 * On the frame path that meant a full surface (640x452 or 640x480 bytes) per
 * submitted frame, including frames the Java side then discards as unchanged.
 * On the phone, 2026-09-16, native-heap growth matched frame bytes submitted
 * to within a few percent (about 350 MB/h with the Ghost window's 3 s poll),
 * and Samsung's Heimdall killed the app at about 6.4 GB. ART also aborts the
 * process once 51,200 global refs are held, so the ref leak was a second
 * ceiling behind the memory one.
 *
